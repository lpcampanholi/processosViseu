...
function SalvarArquivosAtualizados {
  ...
  Get-ChildItem -Path "G:\Drives compartilhados\$Diretorio\$NomeDoAutor\$NomeDoLivro\_Arte\_Físico" -Recurse | Where-Object { $_.FullName -notlike "*\_Descarte*" -and $_.Extension -ne ".indd" } | Move-Item -Destination "G:\Drives compartilhados\$Diretorio\$NomeDoAutor\$NomeDoLivro\_Arte\_Físico\_Descarte" -Force
